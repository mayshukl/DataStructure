...
public class LargestBST {
    ...
    class Value {
        ...
        static int countSizeOptimized(Node node){
            return countSizeOptimized(node,Integer.MIN_VALUE,Integer.MAX_VALUE);
        }

        static int countSizeOptimized(Node node,int min,int max){
            if(node==null){
                return -1;
            }
            
            if(node.data<max&&node.data>min 
                    &&  node.left!=null ? node.left.data < node.data : true
                    && node.right!=null ? node.right.data>node.data : true){
                int leftCount = node.left==null ? 0 : countSizeOptimized(node.left,min,node.data);
                int rightCount = node.right==null ? 0 : countSizeOptimized(node.right,node.data,max);
                if(leftCount==-1||rightCount==-1){
                    return -1;
                }
                return leftCount+rightCount+1;
            }
            int leftCount =  countSizeOptimized(node.left,Integer.MIN_VALUE,Integer.MAX_VALUE);
            int rightCount = countSizeOptimized(node.right,Integer.MIN_VALUE,Integer.MAX_VALUE);
            return leftCount==-1 ? rightCount : Math.max(leftCount,rightCount);
        }
		...
		*/
         //   System.out.println("Size of largest BST is " + tree.largestBST(root));
            System.out.println("Size of largest BST is " + tree.countSizeOptimized(root));
        }
}
